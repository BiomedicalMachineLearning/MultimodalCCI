...
update_CellChat_Db <- function(update_LRs_path){
    ...
    CellChatDB.human.updated$complex <- CellChatDB.human$complex
    CellChatDB.human.updated$cofactor <- CellChatDB.human$cofactor
    CellChatDB.human.updated$geneInfo <- CellChatDB.human$geneInfo
...
# obj <- SCTransform(obj)
obj <- add_image(obj)
data.input = GetAssayData(obj, slot = "data", assay = "Spatial") # normalized data matrix
...
# saveRDS(cellchat,paste0("/scratch/project/stseq/Onkar/BigData/MMCCI/CellChat_v2_",samples[jobid],".rds"))
write.csv(cellchat@dr, file=paste0("/scratch/project/stseq/Levi/mmcci/mmcci_benchmarking/cellchat_v1_",samples[jobid],".csv"))
...
write.csv(cellchat@dr, file=paste0("/scratch/project/stseq/Levi/mmcci/mmcci_benchmarking/cellchat_v2_",samples[jobid],".csv"))